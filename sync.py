...
def fetch_ics_events():
    response = requests.get(ICS_URL, timeout=60, headers={"User-Agent": "Mozilla/5.0"})
...
def sync_to_google(service, ics_events):
    ...
    # First pass: identify all master UIDs
    master_uids = set()
    for event in ics_events:
        if not event["recurrence_id"]:
            master_uids.add(event["uid"])
    
    # Second pass: categorize events
    for event in ics_events:
        if event["recurrence_id"]:
            if event["uid"] in master_uids:
                exception_events.append(event)
            else:
                # Orphan exception - no parent exists, treat as standalone
                orphan_exceptions.append(event)
        else:
            master_events[event["uid"]] = event
    
    print(f"  Masters/Singles: {len(master_events)}, Exceptions: {len(exception_events)}, Orphans: {len(orphan_exceptions)}")
    ...
    # Populate uid_to_google_id from existing events
    for uid, event in existing_events.items():
        if uid not in uid_to_google_id:
            uid_to_google_id[uid] = event["id"]
    
    # ===== PASS 2: Sync orphan exceptions as standalone events =====
    orphans_created = 0
    orphans_updated = 0
    
    for exc_event in orphan_exceptions:
        # Create unique UID for orphan: original_uid + recurrence_id timestamp
        rec_id = exc_event["recurrence_id"]
        if hasattr(rec_id, 'isoformat'):
            rec_id_str = rec_id.isoformat().replace(":", "").replace("-", "").replace("+", "")
        else:
            rec_id_str = str(rec_id)
        orphan_uid = f"{exc_event['uid']}_orphan_{rec_id_str}"
        
        current_uids.add(orphan_uid)
        ...
        try:
            time.sleep(API_DELAY)
            if orphan_uid in existing_events:
                service.events().update(
                    calendarId=GOOGLE_CALENDAR_ID,
                    eventId=existing_events[orphan_uid]["id"],
                    body=google_event
                ).execute()
                orphans_updated += 1
            else:
            ...
            print(f"Error syncing orphan '{exc_event['summary']}': {e}")
            errors += 1
    
    # ===== PASS 3: Sync exception instances (with parents) =====
    ...
    return created, updated, deleted, total_errors, exceptions_updated, orphans_created, orphans_updated
...
def main():
    ...
    try:
        ...
        created, updated, deleted, errors, exceptions_updated, orphans_created, orphans_updated = sync_to_google(service, ics_events)
        ...
        print(f"  Orphan events: {orphans_created} created, {orphans_updated} updated")
